...
from data_utils.Police_Input import PoliceInput  # Ensure this is imported for the test.
...
class IPSReweightingModelTest(tf.test.TestCase, absltest.TestCase):
  ...
  def setUp(self):
    ...
    self.train_steps = 20
    self.test_steps = 5
    self.dataset_base_dir = os.path.join(os.path.dirname(__file__), 'data/police')  # pylint: disable=line-too-long
    self.train_file = [os.path.join(os.path.dirname(__file__), 'data/police/train.csv')]  # pylint: disable=line-too-long
    self.test_file = [os.path.join(os.path.dirname(__file__), 'data/police/test.csv')]  # pylint: disable=line-too-long
    self.load_dataset = PoliceInput(
        dataset_base_dir=self.dataset_base_dir,
        train_file=self.train_file,
        test_file=self.test_file)
    self.label_column_name = 'manner_of_death'  # For the police dataset, this is the target variable.
    self.protected_groups = ['gender', 'race']
  ...
  def test_get_feature_columns_with_demographics(self):
    ...
    self.assertLen(feature_columns, 10)
  ...
  def test_get_feature_columns_without_demographics(self):
    ...
    self.assertLen(feature_columns, 8)
    self.assertEqual(target_variable_column, self.label_column_name)
