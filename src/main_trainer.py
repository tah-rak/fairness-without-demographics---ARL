...
from data_utils.Police_Input import PoliceInput
from data_utils.facebook_input import FacebookInput
...
flags.DEFINE_string("base_dir", "", "Base directory for output.")
...
flags.DEFINE_string("dataset_base_dir", "./data/uci_adult", "(string) path to dataset directory")
flags.DEFINE_string("dataset", "uci_adult", "Name of the dataset to run")
flags.DEFINE_multi_string("train_file", ["./data/uci_adult/adult_data.csv"], "List of (string) path(s) to training file(s).")
flags.DEFINE_multi_string("test_file", ["./data/uci_adult/adult_test.csv"], "List of (string) path(s) to evaluation file(s).")
...
flags.DEFINE_float("primary_learning_rate", 0.01,
...
flags.DEFINE_float("adversary_learning_rate", 2,
...
def get_estimator(model_dir,
  ...
  if model_name == "baseline":
    ...
    print("[DEBUG] Starting ARL training now...")
...
def run_model():
  ...
  if FLAGS.dataset == "law_school":
    ...
    load_dataset = UCIAdultInput(
        ...
        train_file=FLAGS.train_file,
        test_file=FLAGS.test_file)
  elif FLAGS.dataset == "Police":
    load_dataset = PoliceInput(
        dataset_base_dir=FLAGS.dataset_base_dir,
        train_file=FLAGS.train_file,
        test_file=FLAGS.test_file)
  elif FLAGS.dataset == "facebook":
    load_dataset = FacebookInput(
        dataset_base_dir=FLAGS.dataset_base_dir,
        train_file=FLAGS.train_file,
        test_file=FLAGS.test_file)
  else:
