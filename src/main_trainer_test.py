...
def run_experiment(model_name, dataset):
  ...
  FLAGS.dataset_base_dir = os.path.join(os.path.dirname(__file__), 'data/facebook')
  FLAGS.train_file = [os.path.join(os.path.dirname(__file__), 'data/facebook/facebook_train.csv')]
  FLAGS.test_file = [os.path.join(os.path.dirname(__file__), 'data/facebook/facebook_test.csv')]
...
class RunModelTest(tf.test.TestCase, absltest.TestCase):
  ...
  def setUp(self):
    ...
    self._dataset = 'facebook'

  # Test cases for whole training on various datasets
  def test_run_model_on_facebook_dataset(self):
